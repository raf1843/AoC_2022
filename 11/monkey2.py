...
idx = 0
items = {}   # monkey number : items : worry levels
monkeys = {} # monkey number : inspections
ops = {}     # monkey number : worry operations
divs = {}    # monkey number : division test
tosses = {}  # monkey number : toss if True, toss if False
for line in lines:
    if "Monkey" in line:
...
# credit to AoC Reddit for helping me figure this out
# get least common multiple
lcm = 1
for test in divs.values():
    lcm *= test
    ...
    for idx in monkeys: 
        ...
        # generalized a little but assumed operation always assigned to new
            ...
            if ops[idx][1] == '+':
                ...
                items[idx][l] = int(operand1) / int(operand2)
            monkeys[idx] += 1
            # use modulus with least common multiple
            # to lower worry levels while keeping results divisible the same
            items[idx][l] = items[idx][l] % lcm
            # Test
